...
## Palette 1: Pastel Academic *(通用兜底，非永久默认)*
...
## Palette 7: Hand-drawn Watercolor 
...
**核心原则：按这张图的主题领域现挑，不存在「永远的默认」。**

- ML/DL 架构、神经网络                                           → **Palette 1** 或 **Palette 2**
- 提到 Nature/Cell/Science 或"高端期刊"          → **Palette 2**
- 工程/管线/系统/硬件/机器人/控制                       → **Palette 3**
- 生物/化学/临床/湿实验                                            → **Palette 4**或 **Palette 7**
- 图复杂想突出一个重点、做汇报展示                  → **Palette 5**
- 产品文档/blog/演讲/现代风                                   → **Palette 6**

**关于"用户没指定配色"这种情况——这是最容易出错的地方**：

- 用户没说配色，**不等于**让你永远套同一款。你要先看主题落在上面哪一类，**按主题选**。
- 只有当主题确实模糊、归不到任何一类时，才退回 **Palette 1** 作兜底。
- 同一类主题里有两三个候选时（比如 ML 可选 P1/P2），允许换着用，避免给同一用户连画几张图全部撞色。
- 用户**明确**给了配色偏好（"蓝色系""暖一点""跟上次那张一样""我们公司是绿色") → 以用户为准，覆盖上面的主题倾向。

一句话：**配色是一次主动的设计判断，不是查表填空。**
